...
//	@param toAddress
//	@param amount
func Mint(toAddress string, amount int64) {
	// 连接rpc客户端
	...
	tx, err := act.Mint(auth, common.HexToAddress(toAddress), new(big.Int).Mul(decimal, value))
	println("账户:", toAddress, "mint:", value, " 成功! 交易哈希为：", tx)
}
